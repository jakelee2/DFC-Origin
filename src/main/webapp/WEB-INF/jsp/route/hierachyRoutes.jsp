		...
		<section id="content" class="container">
			...
			<div class="block-area" id="job-search">
                ...
                <c:forEach var="selection" items="${selections}">
                    <h3 class="block-title">${selection.getPrefix()}</h3>
                    <datatables:table id="routes_${selection.prefix}" data="${selection.getRoles()}" row="row"
                        cssClass="table tile" cssStyle="true" pageable="false" info="false"
                        export="csv">

                        <datatables:column title="Path" display="html">
                            <spring:url value="/admin/route/${row.getRouteId()}.html" var="routeUrl">
                                <spring:param name="routeId" value="${row.getRouteId()}" />
                            </spring:url>
                            <a href="${fn:escapeXml(routeUrl)}"><c:out
                                    value="${row.getRouteUrl()}" /></a>
                        </datatables:column>
                        <datatables:column title="Level" display="html"><c:out value="${row.getRoleLevel()}" /></datatables:column>
